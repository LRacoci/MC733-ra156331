...
def heatmap(lgX, lgY, Z, labelZ, nome, interp = 'nearest'):
	lgX,lgY = np.meshgrid(lgX,lgY, indexing='ij')
...
def plot2d(X, Y, Z, labelZ, nome):
	...
	ax.scatter(n, Z[...,0], c = 'r', label = "L1i")
	ax.scatter(n, Z[...,1], c = 'g', label = "L1d")
...
for nome in d:
	plot3d(lgx, lgy, 1-d[nome], 'Demand Hit Rate', nome)
	'''
	plot2d(lgx, lgy, 1-d[nome], 'Demand Hit Rate', nome)
	y = 1-d[nome][...,0]
	heatmap(lgx, lgy, y, 'Demand Hit Rate', nome+"_i")
	dy = np.gradient(y)
	dy = np.sqrt(dy[0]**2 + dy[1]**2)
	heatmap(lgx, lgy, dy, 'Demand Hit Rate Change', nome+"_iChange", 'bilinear')
	y = 1-d[nome][...,1]
	heatmap(lgx, lgy, y, 'Demand Hit Rate', nome+"_d")
	dy = np.gradient(y)
	dy = np.sqrt(dy[0]**2 + dy[1]**2)
	heatmap(lgx, lgy, dy, 'Demand Hit Rate Change', nome+"_dChange", 'bilinear')
	'''
