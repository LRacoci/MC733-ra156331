...
do
	...
	do 
        ...
        do
		echo "${ex2}/dinero/d4-7/dineroIV -informat s -trname ${nome[$i]}_f2b -maxtrace ${maxtraces} -l1-isize ${size[$i]}k -l1-dsize ${size[$i]}k -l1-ibsize ${bsize} -l1-dbsize ${bsize} -l1-iassoc ${assoc} -l1-dassoc ${assoc} > $ex2/others/${nome[$i]}_${size[$i]}/bsize${bsize}assoc${assoc}"
